...
솔직한 Q&A — 라즈베리파이가 어디까지 버티는지, 머신 간에 뭐가 동기화되고 뭐가 안 되는지,
왜 회사망을 우회하면 안 되는지, 그리고 머신/계정/워크스페이스/브라우저 중 어느 레이어가
어떤 상태를 소유하는지(다이어그램 포함) — 는 [docs/ko/FAQ.md](docs/ko/FAQ.md)
([English](docs/en/FAQ.md))에 있습니다.

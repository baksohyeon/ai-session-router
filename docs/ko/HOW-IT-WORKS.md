...
`ai`가 속으로 실제 뭘 하는지 쉬운 말로 풀었다. [ARCHITECTURE.md](../ARCHITECTURE.md)가
...
이게 전부다. `ai`는 네가 고른 계정에 맞춰 이 변수를 다른 폴더로 바꿔주고 도구를 실행할 뿐이다.
...
폴더 하나를 서랍 하나라고 보면 된다. `personal`과 `company`는 서랍 두 개다. 도구는 네가
가리킨 서랍만 열어 보므로 둘이 안 섞인다. 개인 결제는 개인에, 회사 기록은 회사에 남는다.

## 명령어 하나 = 서로 무관한 선택 몇 개

`ai` 명령은 사실 서로 영향 안 주는 손잡이 몇 개를 고르는 거다.
...
- **`ai claude` / `ai codex` — 조금 준다.** 워크스페이스로 대신 `cd`해 준다(인자가 정함,
  예: `personal` → `~/dev/personal`). 그 워크스페이스 *밖*에서 시작했으면 경고를 먼저 띄우고,
  그래도 안으로 옮겨 준다.

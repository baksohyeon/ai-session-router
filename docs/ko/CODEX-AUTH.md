...
이 프로세스 수준의 디렉터리 격리가 곧 계정 분리다. CLI에서도 똑같이 동작한다.
...
라우터는 키를 인자로 받지도, 출력하지도, 트랜스크립트에 기록하지도 **않는다**. 위에서
보인 대로 비밀 값은 곧장 `codex`로 파이프하라. 그러면 라우터를 거치지 않는다.
어떤 관리자 통제, 데이터 보존, 데이터 소재지(residency), RBAC, 결제가 적용되는지는
인증 방식에 따라 다르다. 그건 사용자와 OpenAI 사이의 문제고, 라우터는 *어느* 신원이
활성인지만 고정한다.

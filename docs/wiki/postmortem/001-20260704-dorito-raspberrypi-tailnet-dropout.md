...
- [ ] **서버 역할 기기는 Tailscale 키 만료를 끈다.** 관리 콘솔 → Machines →
      raspberrypi → Disable key expiry. 안 하면 180일 뒤 오늘이 반복된다.
- [ ] **파이 정품 전원.** 상시 켜둘 기기가 상시 안 켜져 있으면 전원부터 의심.
...
| **서버 기기 키 만료 끄기** | 관리 콘솔은 코드 밖 → 문서 §1.6/§7.2에 명시했으나 **사용자 수동 작업 미완** | ⏳ 사용자 액션 대기 |
| **파이 정품 어댑터 교체** | 물리 작업 | ⏳ 사용자 액션 대기 |

승격 안 된 두 줄(키 만료 끄기 · 어댑터)은 **사용자만 할 수 있는 물리/콘솔 작업**이라
코드/문서로 못 내린다. 문서에 근거는 심어뒀으니, 남은 건 실행뿐.

...
이 문서는 일부러 **개념 먼저**로 갑니다. 시중의 "이거 그냥 따라 치세요" 가이드가
...
집 Wi-Fi 노트북은 보통 `192.168.x.x`를 받습니다. 그 주소는 외부에서 보면 아무
의미 없음.
...
핵심 결과: **나가는 연결은 되는데, 들어오는 연결은 안 됩니다.** 인터넷의 어떤
서버가 `너희집공인IP`로 패킷을 보내도 공유기는 "이 패킷을 누구한테 보내야 하지?"를
모르고 그냥 버립니다.

이게 "기차에서 그냥 SSH 쳐서 집 컴퓨터 들어가야지"가 기본값으로는 안 되는 이유.
...
프로그램이 포트에서 **listen**합니다. 다른 프로그램이 `(ip, 포트)` 쌍으로
**connect**합니다. 아무도 듣고 있지 않으면 연결은 즉시 거부됨.
...
다 동작합니다. 다 셋업 많이 들거나, 공격면이 넓거나, 특정 벤더에 묶입니다.
그래서 mesh VPN. §5 참조.

### 1.5 CGNAT — 공인 IP조차 없는 경우

일부 ISP (모바일, 한국 일부 알뜰/특정 통신사 광케이블)는 공인 IP조차 안 줍니다.
*통신사 차원의* NAT 뒤에 둠. 포트 포워딩 자체가 불가능. Tailscale 같은 mesh VPN이
잘 동작하는 이유는 **들어오는 연결이 필요 없기** 때문 — 양쪽 다 바깥으로 나가서
중간 릴레이에서 만남.
...
IPv6는 모든 기기에 글로벌 유일 주소를 줘서 이론적으로 NAT가 필요 없어집니다.
실제로는 한국 가정용 IPv6 보급이 아직 들쭉날쭉이고, 방화벽이 어차피 들어오는
트래픽 막습니다. IPv6가 §1.2를 해결해줄 거라 기대하지 마세요.
...
`ssh-agent`는 복호화된 개인키를 메모리에 들고 있는 백그라운드 프로세스. 세션당
패스프레이즈를 한 번만 치게 해줌.
...
이제 `ssh home`이면 끝. `ProxyJump`는 점프 박스를 자동으로 통과시켜줌.
...
[Mosh](https://mosh.org/)는 SSH 세션을 UDP 기반으로 대체합니다 (로그인은 SSH로
함). IP 바뀌어도, 화면 잠금돼도, 패킷 좀 깨져도 살아남음. 휴대폰 LTE에서 SSH
쓰는 거면 일반 SSH보다 압도적으로 나음. 양쪽 다 `brew install mosh`. 방화벽은
UDP 60000–61000을 열어줘야 함.

Tailscale + mosh + tmux 조합이 모바일 끝판왕.
...
tmux는 SSH 연결보다 *오래 살아남는* **서버 위 영속 세션**을 만들어줌. 거기에
attach해서 작업하고, detach함. 셸과 그 안의 모든 것은 계속 돌고 있음.
...
- 호스트가 **재부팅**되면 tmux 세션 다 사라짐. 디스크에 안 저장됨.
  중요하면 [tmux-resurrect](https://github.com/tmux-plugins/tmux-resurrect).
- 부모 터미널을 닫아도 tmux는 죽지 않음. 그게 본질이니까.
- tmux 세션은 `tmux`를 실행한 **사용자**에 묶임. 다른 사용자 세션에 권한 없이는
  못 들어감.
- tmux ≠ screen. 2010년대 `screen` 기억하는 사람이면, tmux가 그 후속.
...
잠든 맥은 tmux 세션을 못 돌림. CPU 자체가 명령어 실행을 안 하니까.
...
`pmset` 설정은 재부팅해도 유지됨. AC 전원에서는 절대 안 자는 *서버 같은* 맥을
원하면 이거. 한 번만, 특정 작업 동안만 막고 싶으면 `caffeinate`.
...
`sudo pmset -a disablesleep 1` 같은 핵으로 다 우회할 수도 있지만 비추. 가방
안에서 노트북 뜨거워지고 배터리 빨려요. 합리적인 옵션:
...
`caffeinate`와 동등한 동작에 규칙 UI를 입혔다고 보면 됨. 기능상 슈퍼셋이지만 한 번
쓰고 마는 경우엔 CLI가 빠름.
...
[Tailscale](https://tailscale.com)은 **WireGuard 위에 만든 mesh VPN**. 본인 모든
기기에 깔고 같은 계정으로 로그인하면, 모든 기기가 같은 LAN에 있는 것처럼 서로
이름으로 도달 가능. 어디서든.

### 5.2 왜 포트 포워딩 없이 동작하나

양쪽 기기가 Tailscale 코디네이션 서비스로 **나가는** 연결을 만듦. NAT 트래버설
기법 (STUN, hole punching)으로 대부분의 경우 두 기기가 **직접** 연결되게 해줌.
직접 안 되면 (일부 symmetric NAT, 회사 방화벽) **DERP 릴레이**가 TCP/443으로
폴백 — 웹 브라우징 허용하는 네트워크면 다 통과.

결론: 공유기에서 포트 안 엽니다. §1.2의 "들어오는 연결" 문제가 사라짐.
...
이제 같은 tailnet에 로그인된 어떤 기기에서든 `ssh user@home-mac`가 키 관리 없이
성공. 인증은 어드민 콘솔에서 짜는 ACL이 결정. 이걸 Tailscale 계정의 2FA랑 조합
(check ACL 규칙에 필요) 하면 현실적으로 가장 강함.
...
내 모든 트래픽이 특정 위치 (지오락 서비스 우회용 집 IP 같은)에서 나가는 것처럼
보이게 하고 싶을 때 유용. **회사 기기를 개인 exit으로 쓰지 마세요** — 신원이
섞임. 그게 `ai` 라우터가 막으려는 그 문제.
...
`check`는 "허용하되 N시간마다 SSO 재인증 묻기". 할 만함.
...
올바른 방법: 키를 **휴대폰에서** 생성, 공개키를 서버로 복사, 개인키는 휴대폰에만
둠. 잘못된 방법: 노트북 개인키를 클라우드 동기화 메모 앱에 붙여서 휴대폰에 옮김.

휴대폰 잃어버리면, 서버 `authorized_keys`에서 그 휴대폰 공개키만 지우거나
(Tailscale SSH면 tailnet에서 그 기기 빼면) 다른 기기엔 영향 없이 정리됨.

### 6.4 "휴대폰 키보드 안 좋잖아" 현실

30초 이상 칠 거면 블루투스 키보드가 휴대폰을 진짜 쓸만한 미니 노트북으로
만듦. 자주 할 거면 작은 접이식 키보드 하나 가방에 넣어둘 만함.
...
지하철 들어가서 연결 끊김? 다시 연결, `tmux attach -t claude`, 그대로 복귀. Wi-Fi
...
대부분의 회사는 의도적으로 막아두며, 우회 시도는 징계 사유. **하지 마세요.**
원격 근무가 공식 허용이면 회사가 수단을 제공함 (회사 VPN, VDI). 그걸 쓰세요.
...
처리. 합치면 노트북 닫고, 터널 들어가고, Wi-Fi → LTE 갈아타고, 20분 뒤 카페에서
열어도 세션이 그대로 있음.
...
올바른 동선: 보안/IT 팀에 글로 물어보기. "내 개인 노트북에 Tailscale" → 거의 항상
*예*. "회사 노트북에" → 거의 항상 *경우에 따라 다름*. 그 대화 자체가 본인을 트러블
밖에 둠.
...
- 로그는 AI 도구가 돈 호스트의 **워크스페이스**에 기록됨 — 즉 서버에 남지 휴대폰에
  안 남음. 그게 의도이자 올바름.
...
이 레포의 `ai` 명령어는 이미 계정/워크스페이스/브라우저를 분리. 원격 접근은
*어느 호스트인지*를 그 위에 더하는데, 축은 그대로 직교 유지.
...
라우터 로그는 늘 그렇듯 `<workspace>/.ai-logs/...` *ai가 돈 호스트*에 남음 — 즉 집
맥에 남지 휴대폰에 안 남음. 그게 의도.
...
이제 `home-ai`만 치면 어디서든 집 맥에 들어가서 알맞은 계정/워크스페이스로
Claude tmux 세션에 안착.
...
이건 의도된 경계: 라우터는 VPN 셋업이나 잠자기 정책, 원격 프로세스 관리의
책임을 질 자리가 아님. 그런 건 이 문서의 도구들이 함. `ai`는 신원 라우팅만 함.
...
- 이 레포의 [ARCHITECTURE.md](./ARCHITECTURE.md), [COMMANDS.md](./COMMANDS.md),
...
**CGNAT면 포트 포워딩 자체가 불가능** → Tailscale (또는 다른 mesh VPN)만 답.
다행히 Tailscale은 CGNAT 뒤에서도 잘 동작 — 그게 §1.5의 핵심.
...
→ **회사 노트북에 Tailscale 깔 생각 자체를 권하지 않음.** 망분리 환경이면 IT
보안팀에 공식 요청해서 그쪽에서 제공하는 수단 (회사 VDI, 회사 Tailscale 테넌트
등)을 쓰세요. 우회 시도는 본인 커리어에 안 좋음.
...
→ Claude/Codex에 한글 프롬프트 칠 거면 **블루투스 키보드 강추**. 화면 키보드로
긴 한글 명령 치는 건 진짜 고통.
...
직접 연결이 안 되면 보통 양쪽 NAT 종류 (symmetric NAT) 문제. 공유기 UPnP 켜거나
포트 포워딩 한 줄 (UDP 41641) 해주면 P2P 성공률 올라감.
